...
# __NAMESERVER__, __XRDP_INSTALLER_URL__, __XRDP_VER__
...
runcmd:
  ...
  - unzip -o "xrdp-installer-__XRDP_VER__.zip" -d /tmp/xrdp-install
  - chmod +x "/tmp/xrdp-install/xrdp-installer-__XRDP_VER__.sh"
  # Run installer in non-interactive mode: option 3 = xfce
  - echo "3" | bash "/tmp/xrdp-install/xrdp-installer-__XRDP_VER__.sh" || true
  # Apply low-encryption config
  - bash /tmp/fix-xrdp-ini.sh
  # ── podman rootless ─────────────────────────────────────
  - bash /tmp/setup-podman-rootless.sh
  # ── Cleanup ─────────────────────────────────────────────
  - rm -rf /tmp/xrdp-install /tmp/fix-xrdp-ini.sh /tmp/setup-podman-rootless.sh
